...
MOL_EDGE_LIST_FEAT_MTX = {drug_id: mol for drug_id, mol in MOL_EDGE_LIST_FEAT_MTX.items() if mol is not None}  ####删除分子图中没有转成边的药物

TOTAL_ATOM_FEATS = (next(iter(MOL_EDGE_LIST_FEAT_MTX.values()))[1].shape[-1])  ###shape【-1】 所有维度的最后一位，二维数组的话是列，每一个药物的特征的维度


##### DDI statistics and counting #######
df_all_pos_ddi = pd.read_csv('/home/disk2/fengyuehua/PycharmProjects/paper4/data/ddis1.csv')
